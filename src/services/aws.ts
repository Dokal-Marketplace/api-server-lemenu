import {
  ...
  async function example() {
    ...
    const textractConfig: TextractConfig = {
      ...
      maxRetries: 3,
      useAnalyze: false // Set to true for better structure detection
    };
  
    // Create parser
    const parser = new MenuParser(textractConfig);
  
    // Option 1: Parse from local file/URL
    const menu = await parser.parseMenuFromImage('./menu.jpg');
  
    // Option 2: Parse from S3
    // const menu = await parser.parseMenuFromS3('my-bucket', 'menus/menu.jpg');
  
    // Display results
    console.log('Restaurant:', menu.restaurantName);
    console.log('OCR Provider:', menu.metadata.ocrProvider);
    console.log('Total Items:', menu.metadata.totalItems);
    console.log('Average Confidence:', menu.metadata.averageConfidence + '%');
    console.log('Processing Time:', menu.metadata.processingTimeMs + 'ms');
    ...
    for (const section of menu.sections) {
      ...
      for (const item of section.items) {
        ...
        console.log(`    (Confidence: ${Math.round(item.confidence)}%)`);
      }
    }
  
    // Export options
    const json = parser.exportToJSON(menu);
    const csv = parser.exportToCSV(menu);
